...
contract AnonifyWithTreeKem is ReportHandle {
    ...
    event StoreTreeKemCiphertext(bytes ciphertext, uint256 stateCounter, bytes16 _traceId);
    event StoreTreeKemHandshake(bytes handshake, uint256 stateCounter, bytes16 _traceId);
    ...
    function joinGroup(
        ...
        storeTreeKemHandshake(_handshake, _traceId);
    }
    ...
    function storeCommand(
        ...
        emit StoreTreeKemCiphertext(_newCiphertext, incremented_state_counter, _traceId);
    }
    ...
    function storeTreeKemHandshake(bytes memory _handshake, bytes16 _traceId) private {
        uint256 incremented_state_counter = _stateCounter.add(1);
        _stateCounter = incremented_state_counter;
        emit StoreTreeKemHandshake(_handshake, incremented_state_counter, _traceId);
    }
}
