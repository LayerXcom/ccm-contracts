...
contract AnonifyWithTreeKem is ReportHandle {
    ...
    event StoreTreeKemCiphertext(bytes ciphertext, uint256 stateCounter, bytes16 _requestId);
    event StoreTreeKemHandshake(bytes handshake, uint256 stateCounter, bytes16 _requestId);
    event UpdateMrenclaveVer(uint32 newVersion, bytes16 _requestId);
    ...
    function joinGroup(
        ...
        storeTreeKemHandshake(_handshake, _requestId);
    }
    ...
    function updateMrenclave(
        ...
        storeTreeKemHandshake(_handshake, _requestId);
        emit UpdateMrenclaveVer(_newVersion, _requestId);
    }
    ...
    function storeCommand(
        ...
        emit StoreTreeKemCiphertext(_newCiphertext, incremented_state_counter, _requestId);
    }
    ...
    function storeTreeKemHandshake(bytes memory _handshake, bytes16 _requestId) private {
        uint256 incremented_state_counter = _stateCounter.add(1);
        _stateCounter = incremented_state_counter;
        emit StoreTreeKemHandshake(_handshake, incremented_state_counter, _requestId);
    }
}
